...
bool Math::Camera::operator==(Camera const& camera) const
{
    auto currentPosition = GetPosition();
    auto prevPosition = camera.GetPosition();
    auto currentRotation = XMVECTOR(GetRotation());
    auto prevRotation = XMVECTOR(camera.GetRotation());

    return (
        static_cast<float>(currentPosition.GetX()) == prevPosition.GetX() &&
        static_cast<float>(currentPosition.GetY()) == prevPosition.GetY() &&
        static_cast<float>(currentPosition.GetZ()) == prevPosition.GetZ() &&
        currentRotation.m128_f32[0] == prevRotation.m128_f32[0] &&
        currentRotation.m128_f32[1] == prevRotation.m128_f32[1] &&
        currentRotation.m128_f32[2] == prevRotation.m128_f32[2] &&
        currentRotation.m128_f32[3] == prevRotation.m128_f32[3]);
}

bool Math::Camera::operator!=(Camera const& camera) const
{
    return !(this->operator==(camera));
}
