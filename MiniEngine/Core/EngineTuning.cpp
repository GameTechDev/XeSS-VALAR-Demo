...
void EngineTuning::Display( GraphicsContext& Context, float x, float y, float w, float h )
{
    ...
    */
    ...
    if (GameInput::IsPressed(GameInput::DigitalInput::kKey_lshift) ||
        GameInput::IsPressed(GameInput::DigitalInput::kKey_rshift))
    {
        Context.SetScissor((uint32_t)Floor(x), (uint32_t)Floor(y), (uint32_t)Ceiling(x + w),
            (uint32_t)Ceiling(y + h));
