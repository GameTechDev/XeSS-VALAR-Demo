...
#include "VRS.h"
#include "..\..\Source\VRSTest.h"
#include "..\..\Source\DemoApp.h"
...
void PostEffects::Render( void )
{
    ...
    if (Graphics::IsUpscalingEnabled())
    {
        ...
        Context.TransitionResource(
            g_UpscaledSceneColorBuffer, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE |
    ...
    VRS::Render(Context,
        Graphics::IsUpscalingEnabled() || VRSTest::m_App->GetTechnique() == kDemoTech_TAAScaled);
    ...
    if (Graphics::IsUpscalingEnabled())
    {
        wasScreenshotTaken = VRSTest::Render(Context, g_UpscaledSceneColorBuffer, g_VRSTier2Buffer);
    }
    else
    {
        if (VRSTest::m_App->GetTechnique() == kDemoTech_TAAScaled)
            wasScreenshotTaken =
                VRSTest::Render(Context, g_UpscaledSceneColorBuffer, g_VRSTier2Buffer);
        else
            wasScreenshotTaken = VRSTest::Render(Context, g_SceneColorBuffer, g_VRSTier2Buffer);
    }
