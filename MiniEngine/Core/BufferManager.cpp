...
void Graphics::InitializeRenderingBuffers( uint32_t bufferWidth, uint32_t bufferHeight )
{
    ...
    esram.PushStack();
        ...
        {
            g_VRSTier2Buffer.Create(L"Shading Rate Image Buffer",
                (uint32_t)round(bufferWidth / (float)VRS::ShadingRateTileSize),
                (uint32_t)round(bufferHeight / (float)VRS::ShadingRateTileSize), 1,
                DXGI_FORMAT_R8_UINT);
